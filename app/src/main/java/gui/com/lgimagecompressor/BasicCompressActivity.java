...
public class BasicCompressActivity extends AppCompatActivity implements LGImgCompressor.CompressListener{
    ...
    private final static int CAMERA_REQESTCODE = 100; //requstcode
    ...
    private void requestPermission() {
        if (ContextCompat.checkSelfPermission(this, android.Manifest.permission.WRITE_EXTERNAL_STORAGE)!= PackageManager.PERMISSION_GRANTED) { //granted

            ActivityCompat.requestPermissions(this,new String[]{android.Manifest.permission.WRITE_EXTERNAL_STORAGE},CAMERA_REQESTCODE);
        } else {
            takePictureFormCamera();
        }
    }

    @Override  //这个是动态权限写入以后的回调
    ...
    private File imageFile; //-------- /storage/emulated/0/Pictures/20160701175916_1810892596.jpg
    //相机拍照
    private void takePictureFormCamera() {
        ...
        File fileDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES); //下载的图片保存的位置
        imageFile = null;
        try {
