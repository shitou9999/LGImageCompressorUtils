...
public class LGImgCompressor {
     ...
     *LGImgCompressor.getInstance(this).withListener(this). starCompress(Uri.fromFile(imageFile).toString(),outWidth,outHeight,maxFileSize);来启动压缩任务
     */
    ...
    private class CompressTask extends AsyncTask<String, Void, CompressResult> {
        ...
        protected void onPostExecute(CompressResult compressResult) {
            if (compressListener != null) {
                compressListener.onCompressEnd(compressResult);//监听回调（压缩结束）
            }
        }
    }
}
